...
IFileInfo** FindFiles(const char* aFolder)
{
    ...
    for (size_t i = 0; i < sizeof(lPattern); i++)
    {
        std::cout << lPattern[i] << "\n";
    }
...
IFileInfo* CreateFileInfo(const WIN32_FIND_DATA& aData)
{
   /* size_t lLen = strlen(aData.cFileName);
    IFileInfo* lResult;
    if 
        (0 == _stricmp(".cpp", aData.cFileName + lLen - 4)) { lResult = new FileInfo_CPP; }
    else if
        (0 == _stricmp(".exe", aData.cFileName + lLen - 4)) { lResult = new FileInfo_EXE; }
    else if
        (0 == _stricmp(".h", aData.cFileName + lLen - 2)) { lResult = new FileInfo_H; }
    else
    {
